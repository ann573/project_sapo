  ...
  return (
    <div className="px-2">
      ...
      <div className="flex justify-between text-sm font-semibold my-1">
        <p>Khách phải trả: </p>
        <p>{endInvoice.total.toLocaleString('vi', {style : 'currency', currency : 'VND'})}</p>
      </div>
      <div className="flex justify-between text-xs my-1">
        <p>Tiền khách đưa: </p>
        <p className="flex gap-[3px]">{endInvoice.givenMoney  || endInvoice.total}₫</p>
      </div>
      <div className="flex justify-between text-xs my-1">
        <p>Tiền thừa: </p>
      ...
      <hr className="my-5"/>
      <p className="text-center text-sm italic">Cám ơn quý khách đã tin tưởng mua hàng</p>
    </div>
  );
};
